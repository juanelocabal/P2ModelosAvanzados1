...
def predict_price(year,mileage,state,make,model):
    ...
    for si in stc:
        if si.upper().strip()==state.upper().strip():
            car_[si]=1
        else:
            car_[si]=0
    for mki in mkc:
        if mki.upper().strip()==make.upper().strip():
            car_[mki]=1
        else:
            car_[mki]=0

    for mdli in mdlc:
        if mdli.upper().strip()==model.upper().strip():
            car_[mdli]=1
        else:
            car_[mdli]=0
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print('Please add all parameters')
        print(sys.argv)
    else:
